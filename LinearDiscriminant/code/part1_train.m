function [] = part1_train%(testfile)      % takes input as testsample filename which must contain testdata and testlabel
...
%    testlabel = s.testLabel;
    ...
    norm_A = [];
    for i=1:size(A,1),
        norm_A = [norm_A;A(i,:)/norm(A(i,:))];      %normalizing a's
    end
    
    csvwrite('part1_A.csv',norm_A);     %writing the a values in a graph
